...
export default function PlayHealth() {
  ...
  // Ensure the "current" object exists (no calculations here—just a shape)
  useEffect(() => {
    if (!charData.current) {
  ...
  return (
    <section className="rounded-2xl border border-slate-700 bg-slate-800/40 p-4 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
        ...
        {/* Temp HP (user-entered, posted raw) */}
        <div className="flex items-center gap-3">
        ...
        {/* Amount + action buttons (commands only) */}
        <div className="flex items-center gap-2">
          <input
            type="number"
            min={0}
          ...
          <button
            ...
            title="Tell backend to apply damage"
          >
            Damage
          </button>
          <button
            ...
            title="Tell backend to apply healing"
          >
            Heal
          </button>
        </div>
      ...
      {/* Quick buttons – send commands only */}
