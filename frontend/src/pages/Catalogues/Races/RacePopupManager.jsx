...
export function useRacePopups() {
  const [openPopups, setOpenPopups] = useState([]);
  const [minimized, setMinimized] = useState([]);
  const zCounter = useRef(100);

  const openRace = (race) => {
    const id = `${race.name}__${race.source}`;
    if (minimized.some((p) => p.id === id)) { restore(id); return; }
    if (openPopups.some((p) => p.id === id)) { focus(id); return; }
  ...
  const focus = (id) => {
    ...
    setOpenPopups((prev) => prev.map((p) => (p.id === id ? { ...p, zIndex: z } : p)));
  };

  const move = (id, x, y) =>
    setOpenPopups((prev) => prev.map((p) => (p.id === id ? { ...p, x, y } : p)));

  const resize = (id, width, height) =>
    setOpenPopups((prev) => prev.map((p) => (p.id === id ? { ...p, width, height } : p)));

  return { openPopups, minimized, openRace, close, minimize, restore, focus, move, resize };
}

export default function RacePopupLayer({
  popups, minimized, onClose, onMinimize, onRestore, onFocus, onMove, onResize,
}) {
  ...
  const handleResizeStart = (e, win) => {
    ...
    const startX = e.clientX, startY = e.clientY;
    const originW = win.width, originH = win.height;
        ...
        >
          ...
          >
            ...
            <div className="flex gap-1">
              <button className="px-2 hover:bg-stone-300 rounded text-stone-700"
                      onClick={() => onMinimize(win.id)}>−</button>
              <button className="px-2 hover:bg-red-300 rounded text-stone-700"
                      onClick={() => onClose(win.id)}>×</button>
            </div>
          </div>

          <div className="flex-1 overflow-hidden">
            <RacePopup race={win.race} />
          </div>
